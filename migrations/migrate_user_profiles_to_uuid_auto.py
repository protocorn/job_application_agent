"""
Migration: Convert user_profiles.id from Integer to UUID (Automatic Version)

This migration automatically detects existing columns and migrates them properly.
Runs without interactive confirmation.

Run with: python migrations/migrate_user_profiles_to_uuid_auto.py
"""
...
def get_existing_columns(connection):
    """Get list of columns that exist in the backup table"""
...
def migrate_user_profiles_to_uuid():
    ...
    try:
        ...
        # Handle COALESCE for JSONB columns and type conversions
        jsonb_array_columns = ['other_links', 'education', 'work_experience', 'skills',
                               'projects', 'certifications', 'languages', 'disabilities',
                               'preferred_location']
        jsonb_object_columns = ['job_preferences']
        boolean_columns = ['willing_to_relocate', 'willing_to_travel']

        select_parts = []
        for col in restore_columns:
            if col in jsonb_array_columns:
                # Cast JSON to JSONB if needed
                select_parts.append(f"COALESCE({col}::jsonb, '[]'::jsonb)")
            elif col in jsonb_object_columns:
                # Cast JSON to JSONB if needed
                select_parts.append(f"COALESCE({col}::jsonb, '{{}}'::jsonb)")
            elif col in boolean_columns:
                # Cast VARCHAR to BOOLEAN (handle 'true'/'false' strings or NULL)
                select_parts.append(f"CASE WHEN {col} = 'true' THEN true WHEN {col} = 'false' THEN false ELSE NULL END")
            else:
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("USER_PROFILES TABLE MIGRATION TO UUID (AUTOMATIC)")
    print("="*60)
    print("\nThis will:")
    ...
    try:
        if migrate_user_profiles_to_uuid():
            if verify_migration():
                print("\n" + "="*60)
