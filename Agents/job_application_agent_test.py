...
# Hidden browser manager import (same directory)
try:
    from hidden_browser_manager import HiddenBrowserManager
except ImportError:
    # Fallback if import fails
    logger.warning("Could not import HiddenBrowserManager - hidden browser mode disabled")
    HiddenBrowserManager = None
...
class RefactoredJobAgent:
    ...
    def __init__(self, playwright, headless: bool = True, keep_open: bool = False, debug: bool = False, hold_seconds: int = 0, slow_mo_ms: int = 0, job_id: str = None, jobs_dict: dict = None, session_manager: SessionManager = None, user_id: str = None, use_hidden_browser: bool = True) -> None:
        self.playwright = playwright
        
        # Hidden browser mode REQUIRES headful mode (visible window that can be minimized)
        if use_hidden_browser and headless:
            logger.warning("⚠️ Hidden browser mode requires headful mode (visible window)")
            logger.info("🔄 Automatically switching to headful mode for hidden browser feature")
            headless = False
        ...
        self.browser = None  # Store browser reference for hidden mode
        self.current_context: Optional[Union[Page, Frame]] = None
        self.state_machine: Optional[StateMachine] = None
        self.use_hidden_browser = use_hidden_browser  # NEW: Use hidden browser approach
        
        # Initialize hidden browser manager with error handling
        self.hidden_browser_manager = None
        if use_hidden_browser and HiddenBrowserManager is not None:
            try:
                self.hidden_browser_manager = HiddenBrowserManager()
                logger.info("✅ Hidden browser manager initialized")
            except Exception as e:
                logger.error(f"❌ Failed to initialize hidden browser manager: {e}")
                logger.warning("⚠️ Falling back to standard browser behavior")
                self.use_hidden_browser = False
        elif use_hidden_browser and HiddenBrowserManager is None:
            logger.warning("⚠️ HiddenBrowserManager not available - disabling hidden browser mode")
            self.use_hidden_browser = False
    ...
    async def _new_page(self) -> Page:
        ...
        self.browser = browser  # Store browser reference for hidden mode
    ...
    async def process_link(self, url: str) -> None:
        ...
        try:
            ...
            # Skip cleanup if using hidden browser mode (browser stays alive)
            if self.use_hidden_browser and hasattr(self, 'keep_browser_open_for_human') and self.keep_browser_open_for_human:
                logger.info("🧊 Hidden browser mode - skipping cleanup, browser stays alive")
                return
...
"""
    ...
    async def _state_human_intervention(self, state: ApplicationState) -> Optional[str]:
        """Pauses execution - hides browser or waits for input based on mode."""
        ...
        # Debug logging
        logger.info(f"🔍 Hidden browser mode check: use_hidden_browser={self.use_hidden_browser}, manager_exists={self.hidden_browser_manager is not None}")
        
        # SIMPLIFIED PAUSE MODE (BETA - Works reliably!)
        if self.use_hidden_browser:
            print("\n🛑 PAUSED - WAITING FOR YOUR INPUT (BETA)")
            print("   Browser is visible - complete the required action")
            print("="*50 + "\n")
            
            print("💡 Your options:")
            print("   [Enter]  - I completed the action, let agent continue")
            print("   [m]      - I'll finish manually, keep browser open and exit")
            print("   [q]      - Quit and close browser")
            print()
            
            self._log_to_jobs("info", f"⏸️ Paused - Waiting for user input")
            
            try:
                choice = await asyncio.get_event_loop().run_in_executor(
                    None,
                    input,
                    "Your choice (Enter/m/q): "
                )
                choice = choice.strip().lower()
                
                if choice == 'm':
                    # Manual mode - keep browser open, exit script
                    print("\n✅ Browser will stay open for manual completion")
                    print("   Complete the application and close browser when done!")
                    self.keep_browser_open_for_human = True
                    self._log_to_jobs("info", "👤 User will complete manually")
                    return None  # Exit state machine
                    ...
                    # Continue - user completed the action
                    print("\n✅ Continuing...")
                    self._log_to_jobs("info", "✅ User completed intervention - agent continuing")
                    state.context.pop('came_from_human_intervention', None)
                    return 'ai_guided_navigation'  # Let agent continue
                    
            except KeyboardInterrupt:
                print("\n\n👋 Ctrl+C detected. Keeping browser open.")
                self.keep_browser_open_for_human = True
                return None
        
        # FALLBACK: MANUAL INTERVENTION MODE (if hidden browser disabled)
...
async def run_links_with_refactored_agent(links: list[str], headless: bool, keep_open: bool, debug: bool, hold_seconds: int, slow_mo_ms: int, job_id: str = None, jobs_dict: dict = None, session_manager: SessionManager = None, user_id: str = None):
    p = await async_playwright().start()
    try:
        # ALWAYS create session manager if not provided (for standalone mode)
        if session_manager is None:
            logger.info("🔧 No session manager provided - creating one for action recording")
            session_manager = SessionManager(storage_dir="sessions")
            logger.info("✅ Session manager created - actions will be recorded")
        
        agent = RefactoredJobAgent(p, headless=headless, keep_open=keep_open, debug=debug, hold_seconds=hold_seconds, slow_mo_ms=slow_mo_ms, job_id=job_id, jobs_dict=jobs_dict, session_manager=session_manager, user_id=user_id)
        for link in links:
        ...
        if not should_keep_open:
            ...
            # Check if we're in hidden browser mode
            if hasattr(agent, 'use_hidden_browser') and agent.use_hidden_browser and hasattr(agent, 'keep_browser_open_for_human') and agent.keep_browser_open_for_human:
                # Hidden browser mode - user already pressed Ctrl+C once, don't loop again
                logger.info("🧊 Hidden browser mode - browser stays alive in background")
                logger.info("💡 Resume with: python Agents/resume_hidden.py")
                # Don't stop playwright - browser stays alive
            else:
                # Regular keep-open mode (debug or --keep-open flag)
